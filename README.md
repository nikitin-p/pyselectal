...
`-o, --output PATH` — Output file path (for `--select` and `--count`) or directory (for `--all`). Overrides automatic naming. For `--all`, the directory is created if it does not exist; for `--select` and `--count`, the parent directory must already exist.
